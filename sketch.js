...
function setup() {
  ...
  pts = [];
  for (let i = 0; i < 6; i++){
    pts.push({x: scale*cos(60*i), y: scale*sin(60*i)});
  }
  h = new sixgon(pts,false,0);
  hexes = makeFlake(h,10);
}
...
function draw() {
  ...
  rotate(timer/4);
  for (let i = 0; i < min(hexes.length,timer/2); i++) {
    for (let j = 0; j < 6; j++){
      hexes[i].display();
      rotate(60);
    }
...
class sixgon {
  constructor(verts, plate, level) {
    ...
    this.vertices = verts; // this is the set of 6 points
    this.plate = plate;
    this.level = level;
  }
